...
async def test_user(client, uid, room_id):
    ...
    if uid % 3 == 0:
        await test_leave(client, uid, room_id)
...
async def test_leave(client, uid, room_id):
    try:
        # 需要足够的时间，让 redis set value
        await asyncio.sleep(9)
        await client.emit("leave", {"room_id": room_id, "user_id": uid}, namespace="/live_socket")
        leave_clients.append(client.sid)
        rooms[room_id] -= 1
    except Exception as ex:
        global fail_leave
        fail_leave += 1
        logger.error(f"failed to leave {client.sid}: {ex}")
