...
parser.add_argument(
    "--r", dest="roundtrips", help="Roundtrips per client", default=1, type=int
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=os.path.join(os.path.dirname(__file__), "log.txt"),
)
...
async def client(state):
    ...
    async with ClientSession() as session:
        async with session.ws_connect(f"http://{host}/ws") as websocket:
            for i in range(roundtrips):
                ...
                await websocket.send_json(
                    {
                        "action": "join",
                        "data": {"room": "235947", "uid": state["clients"]},
                    }
                )
